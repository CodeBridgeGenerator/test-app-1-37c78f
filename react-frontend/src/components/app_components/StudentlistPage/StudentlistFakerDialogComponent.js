...
import studentlistFakerFactory from "./studentlistFakerFactory";
...
const StudentlistFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = studentlistFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("studentlist").create(elem));
...
export default connect(mapState, mapDispatch)(StudentlistFakerDialogComponent);
